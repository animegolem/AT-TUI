...
- [x] FR-1: All `BskyClient` clones shall share one live session; a refresh in any task is visible to all.
- [x] FR-2: Session refresh shall be single-flight; concurrent 401s produce exactly one refresh call.
- [x] FR-3: Consecutive background-poll failures shall surface a persistent statusline indicator; the unread badge shall retain its last known value on error.
- [x] FR-4: Key handling shall ignore non-Press key events.
- [x] FR-5: Mouse wheel shall move the selection up/down.
- [x] FR-6: The terminal shall redraw only when state has changed.
- [x] FR-7: Profile and notification views shall paginate using their stored cursors.
- [x] FR-8: A completing feed load shall replace the root view in place, preserving any pushed views.
- [x] FR-9: The agreed keymap shall be implemented with help overlay and README updated in the same change.
- [x] FR-10: Half-page and page scrolling shall be available in all list views.
- [x] FR-11: Image loads shall check the disk cache before the network.
- [x] FR-12: Images for posts near the selection shall prefetch in the background.
- [x] FR-13: Media previews shall render the thumbnail immediately and upgrade to fullsize.
- [x] FR-14: The in-memory decoded-image cache shall be bounded with LRU eviction.
- [x] FR-15: Outgoing posts shall include link and mention facets.
- [x] FR-16: Post length shall be validated by grapheme count.
- [x] FR-17: Users shall be able to delete their own posts after confirmation.
- [x] FR-18: The statusline pending indicator shall animate (braille spinner) while tasks run.
- [x] FR-19: Quote posts shall render inside box-drawing frames consistent with the app chrome.
...
- [x] [[AI-IMP-002-1-session-sharing-failure-visibility]]: shared session, single-flight refresh, offline indicator (FR-1..3).
- [x] [[AI-IMP-002-2-event-loop-hygiene]]: Press filter, wheel scroll, dirty-flag redraw (FR-4..6).
- [x] [[AI-IMP-002-3-view-pagination-stack-preservation]]: profile/notification pagination, root-in-place feed loads (FR-7..8).
- [x] [[AI-IMP-002-4-keymap-rework-page-scroll]]: agreed keymap plus page motion (FR-9..10).
- [x] [[AI-IMP-002-5-image-cache-prefetch]]: disk read-through, prefetch, thumb-first, LRU (FR-11..14).
- [x] [[AI-IMP-002-6-posting-correctness]]: facets, graphemes, delete own post (FR-15..17).
- [x] [[AI-IMP-002-7-visual-quick-wins]]: braille spinner, box-drawing quote frames (FR-18..19).
